...
const WatchScreen = ({ navigation }) => {
	...
	const fetchData = async (page) => {
		try {
			const resp = await axios.get(
				`${process.env.EXPO_PUBLIC_BASE_URL}/api/users/get-teachers/?page=${page}`,
				{
					headers: {
	...
	const renderLoader = () => {
		return loading ? (
			...
			<Text style={[styles.error, { color: errColor }]}>No more Teachers!</Text>
		) : (
	...
	const renderItem = ({ item }) => {
		return (
			<TouchableOpacity
				activeOpacity={0.9}
				onPress={() => navigation.navigate('Playlists', { userId: item._id })}>
	...
	return (
		...
		<View style={styles.container}>
			<View style={styles.logoContainer}>
				<Text style={styles.searchTitle}>Teachers</Text>
			</View>
