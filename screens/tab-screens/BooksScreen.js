...
const BooksScreen = ({ route, navigation }) => {
	...
	const fetchData = async (page) => {
		try {
			const resp = await axios.get(
				`${process.env.EXPO_PUBLIC_BASE_URL}/api/books/get-books/?page=${page}`,
				{
					headers: {
			...
			setBooks((prev) => (page == 1 ? respData : [...prev, ...respData]));
		} catch (error) {
			console.log(error);
		}
	};
	...
	const renderLoader = () => {
		return loading ? (
			...
			<Text style={[styles.error, { color: errColor }]}>No more Books!</Text>
		) : (
			...
			<TouchableOpacity
				...
				<View style={styles.card}>
					...
					<View style={styles.profileTitle}>
						...
						<Text style={styles.profileTitleSecondary}>{item.description}</Text>
					</View>
				</View>
			</TouchableOpacity>
		);
